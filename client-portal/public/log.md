...
### PoC-Booking-Flow in nachhilfe.html integriert
- **Backups**: `nachhilfe.html.poc-backup` + `nachhilfe.html.prebuild` erstellt
- **CSS-Delta aus PoC**:
  - `.pw-tab-hint` + selected-state (Level-Hinweise: Bachelor-Niveau/Master & MBA/PhD & Promotion)
  - `.bk-tier.featured.selected` (featured+selected Combo)
  - `.bk-dimmed` (Ausgrauung nicht-selektierter Elemente bei Warenkorb)
  - `.bk-tier-name .bk-badge-discount` (Rabatt-Pill inline neben Paketnamen)
  - `.bk-custom-price` font-size 1.05rem → 0.85rem
  - `.bk-custom-label` entfernt
  - `.bk-confirm-area` mit border/bg/padding (wie customPanel)
  - `.bk-tier-arrow` entfernt
  - `.bk-tier` transition auf background/color
- **HTML**:
  - Consulting-Button aus type-grid entfernt (grid auf 2 Spalten)
  - Level-Buttons: `.pw-tab-hint` hinzugefügt
  - Package-Karten: Rabatt-Pills inline ("10 %"/"15 %"), "Empfohlen"-Badge rechts oben
  - Individuell-Karte: "Maßgeschneidert inkl. Rabatte", "Preis berechnen →"
  - confirmArea: "Deine Auswahl" + blauer Divider + border/bg/padding
  - customPanel: "Deine Auswahl" + blauer Divider, Savings-Span unter Stunden-Input
  - IDs: `bkPrice*` → `pkgPrice*`, `#pkgPriceCustom` ergänzt, `#bkStepTitle` für Step-2-Titel
  - Äußeres `bk-title`/`bk-sub` in `bkStep1` verschoben (jedes Panel hat eigene Titel)
- **JS**:
  - PoC-Funktionen (`setDimmed`, `selectLevel`, `selectPackage`, `confirmBooking`, `updateCustomPrice`, `closeCustomPanel`, `submitCustom`) übernommen
  - `bkGoBack()`: 3→2→1 Navigation (Warenkorb → Level-Auswahl → Typ-Auswahl)
  - `bkSelectType()`: Consulting-Fall entfernt, `bkStepTitle` zurücksetzen
  - `formatPrice()`: auf 2 Dezimalstellen vereinfacht
  - Stripe-Calls live: `confirmBooking()` + `submitCustom()` rufen echte `stripeCheckout()`/`stripeCheckoutCustom()` auf (Sandbox-Keys)
- **Translations**:
  - DE: `bk_custom_label` → "Preis berechnen →", `bk_custom_hours_unit` → "Std.", `bk_custom_submit` → "Jetzt verbindlich buchen", `bk_pkg_custom_sub` → "Maßgeschneidert inkl. Rabatte"
  - EN: `bk_custom_label` → "Calculate price →", `bk_custom_hours_unit` → "hrs", `bk_pkg_custom_sub` → "Tailored incl. discounts"
- **Sync**: root `nachhilfe.html` + `translations.js` → `client-portal/public/`

### Offen
1. Nächste Sandbox-Testung des Booking-Flows
2. Stripe-Live-Mode aktivieren nach erfolgreichem Test
